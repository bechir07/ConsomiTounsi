...
public interface PaymentRepository extends CrudRepository<Payment, Integer> {
	...
	@Query(value="SELECT payment_id, user.id, first_name, last_name, bill_id, payment_date, product.name, product.price, amount, total_product_price, payment.total_price, payment.payment_type FROM user INNER JOIN command ON user.id = command.client_id INNER JOIN bill ON command.reference = bill.command_reference INNER JOIN command_line ON command.reference = command_line.command_reference INNER JOIN product ON product.id = command_line.product_id INNER JOIN payment ON bill.bill_id = payment.bill_bill_id ", nativeQuery = true)
	public List<?> getpaymentdetails();
	@Query(value="SELECT payment_id, user.id, first_name, last_name, bill_id, payment_date, product.name, product.price, amount, total_product_price, payment.total_price, payment.payment_type FROM user INNER JOIN command ON user.id = command.client_id INNER JOIN bill ON command.reference = bill.command_reference INNER JOIN command_line ON command.reference = command_line.command_reference INNER JOIN product ON product.id = command_line.product_id INNER JOIN payment ON bill.bill_id = payment.bill_bill_id where user.id=:id", nativeQuery = true)
	public List<?> getpaymentbyclient(@Param("id") int id);
